...
BusyBox.prototype.RenderContent = function () {
    ...
    doc.writeln(" var c=0; var t;");
    doc.writeln(" function timedCount()");
    doc.writeln(" {document.getElementById('txt').value=c;");
    doc.writeln(" c=c+1;t=setTimeout('timedCount()',1000);}");
    doc.writeln(" function startTime()");
    doc.writeln(" {var today=new Date();var h=today.getHours();var m=today.getMinutes();var s=today.getSeconds();");
    doc.writeln("  document.getElementById('txt').value=h+':'+m+':'+s;");
    doc.writeln(" t=setTimeout('startTime()',500);}");
    doc.writeln("</script> ");
    doc.writeln("<body ondragstart='return false;' style='Margin: 0px; Background-Color: white' onload='timedCount();' onbeforeactivate='c=0;'  onresize='c=0;' onactivate='c=0;'  onunload='c=0;' onfocus='c=0;'>");
    doc.writeln("   <div id='" + this.DivID + "' align=center " + style + " >");
    doc.writeln("      <img id='" + this.ImgID + "' src='Images/ajax-loader.gif'>");
    doc.writeln("   <input type='text' id='txt' value='0' style='width: 30; text-align: center; border-style: none;'>");
    doc.writeln("   </div>");
    doc.writeln("</body>");
    doc.close();
}
...
BusyBox.prototype.Resize = function () {
    ...
    if (BusyBox.IsBrowserIE()) {
        ...
        //20180618ㄌ盡竚э
        var ifrmae = document.getElementById('BusyBoxIFrame')
        ifrmae.style.width = '140px'; //this.Width;
        ifrmae.style.height = '110px'; // this.height;

        //琿盿计200  ㄌ夹非 琌璶糶Θ 200px
        // Set the width to the value specified.
        //        this.IFrame.style.width = 200; // this.Width;
        //        this.IFrame.style.height = 200;// this.Height;
    }
}
...
BusyBox.prototype.Center = function () {
    ...
    var objTop = (document.body.clientHeight - this.IFrame.offsetHeight) / 2 + 20;
    //var objTop = 150;
    // objTop = document.documentElement.scrollTop + 150;
    //    objLeft = objLeft + document.body.scrollLeft;
    //objTop = objTop + document.body.scrollTop;


    if (window.console) console.log('scrollTop-->' + objTop + ' ' + objLeft);
    // Position object
...
BusyBox.prototype.Show = function () {
    ...
    if (this.IsAnimating() || this.IsVisible()) {
        console.log(' IsAnimating:{' + this.IsAnimating() + '} IsVisible: {' + his.IsVisible() + '}');
        return;
    }
